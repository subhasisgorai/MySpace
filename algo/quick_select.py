...
def quick_select_kth_min(arr, k):
    ...
    if k <= len(less_than_pivot):
        ...
        return quick_select_kth_min(
            greater_than_pivot, k - len(less_than_pivot) - len(equal_to_pivot)
        )
...
def quick_select_kth_max(arr, k):
    if arr and 0 <= k <= len(arr):
        ...
        if k <= len(greater_than_pivot):
            ...
            return quick_select_kth_max(
                lesser_than_pivot, k - len(greater_than_pivot) - len(equal_to_pivot)
            )
